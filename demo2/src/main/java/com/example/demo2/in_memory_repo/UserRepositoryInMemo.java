...
@Repository
public class UserRepositoryInMemo {
    private Map<String, User> users;

    public UserRepositoryInMemo() {
        users = new HashMap<>();
    }
    ...
    public void followUser(String followerUsername, String followedUsername) {
        ...
        if (follower != null && followed != null) {
            ...
            followingList.add(followed); // Add the followed user to the follower's following list
            follower.setFollowing(followingList);

            List<User> followersList = followed.getFollowers();
            followersList.add(follower); // Add the follower user to the followed user's followers list
